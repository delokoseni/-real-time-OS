...
\begin{itemize}
    \item Использование языка C в RTOS;
    \item Реализация на базе POK kernel;
    \item Обеспечение автоматического восстановления задач;
\end{itemize}
\vspace{0.8em}
\textbf{Основной процесс:} обнаружение сбоя $\rightarrow$ перезапуск задачи.
\end{frame}

\begin{frame}{Реализация: общая идея}
\begin{itemize}
    \item Во время выполнения задача может зависнуть или завершиться с ошибкой;
    \item Система должна отслеживать состояние задач;
    \item При обнаружении сбоя запускается механизм восстановления;
    \item Восстанавливается только сбойная задача.
\end{itemize}
\end{frame}

\begin{frame}[fragile]{Механизм контроля и перезапуска}
\begin{verbatim}
void supervisor() {
